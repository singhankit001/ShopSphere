...
exports.getProducts = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      console.log('MongoDB unavailable, using fallback data with filtering.');
      let filtered = [...fallbackProducts];
      
      if (category && category !== 'all') {
        filtered = filtered.filter(p => p.categorySlug === category || (p.category && p.category.slug === category));
      }
    ...
    if (category && category !== 'all') {
      ...
      if (cat) {
        ...
        // Category not found in DB - return empty result instead of all products
        return res.json({
          products: [],
          page: Number(page),
          pages: 0,
